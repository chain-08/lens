...
class TSXMaxReturnPredictor:
    ...
    def fetch_data_from_clickhouse(self, table_name='tsx_eod'):
        ...
        logging.info(f"✅ Data fetched: {df.shape[0]} rows, {df['date'].min()} to {df['date'].max()}")
        return df
    ...
    def create_features(self, df):
        ...
        self.features = [
            'intraday_return', 'volatility', 'close_to_high', 'close_to_low', 'prev_return',
            'rolling_volatility', 'rolling_return_mean', 'rolling_return_std',
            'log_volume', 'rolling_volume_mean', 'day_of_week', 'month'
        ]
    ...
    def predict_with_rolling_training(self, df, start_date, end_date):
        ...
        for pred_date in date_range:
            train_date = pd.to_datetime(pred_date) - pd.Timedelta(days=1)
            train_df = df[df['date'] <= train_date]
            eval_df = df[df['date'] == pd.to_datetime(pred_date)]

            if eval_df.empty or train_df.empty:
                continue

            X_train, y_train = train_df[self.features], train_df['target']
            X_eval, y_true = eval_df[self.features], eval_df['target']
            ...
            temp_df['SMAPE (%)'] = self.smape(y_true.values * 100, y_pred * 100)
